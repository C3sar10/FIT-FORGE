...
import { Clock, DumbbellIcon, CalendarDaysIcon, ArrowRight, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <div className="w-full h-full flex flex-col items-center px-4 gap-8">
      ...
      <div className="w-full p-4 flex items-center justify-between">
        <span className="w-1/4 inline-flex items-center justify-between">
          ...
          >
            {viewMode === "month" ? <ChevronDown size={28}/> : <ChevronUp size={28}/>}
          </button>
        </span>
          ...
          <ResponsiveContainer width={"90%"} height={300}>
            {viewMode === "month" ? (
              <BarChart data={data}  margin={{ top: 0, right: 20, bottom: 40, left: 20 }}>
                ...
                <BarChart data={yearData} margin={{ top: 0, right: 20, bottom: 40, left: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false}/>
                  <XAxis dataKey="name" label={{ value: year, position: "insideBottom", offset: -20, style: { fill: "#FFFFFF", fontSize: 20 }}} />
                  <YAxis label={{ value: "Training Time (Minutes)", angle: -90, position: "insideLeft", dy: 80, style: { fill: "#FFFFFF", fontSize: 16 }}} />
                  <Tooltip />
