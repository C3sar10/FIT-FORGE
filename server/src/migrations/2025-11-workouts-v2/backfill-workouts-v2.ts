...
function v2PatchForWorkoutItem(item: any) {
  ...
  if (isNil(repObj.repType)) patch.repObj.repType = "number";
  if (isNil(repObj.repNumber)) patch.repObj.repNumber = null;

  if (isNil(repObj.repRange)) patch.repObj.repRange = { min: null, max: null };
  ...
  if (isNil(repObj.equipment)) patch.repObj.equipment = [];

  return patch;
}

function v2PatchForWorkout(doc: any) {
  const patch: any = { schemaVersion: 2, updatedAt: new Date() };

  // Process blocks and their items
  if (doc.blocks && Array.isArray(doc.blocks)) {
    patch.blocks = doc.blocks.map((block: any) => ({
      ...block,
...
(async () => {
  ...
  while (true) {
    ...
    if (ops.length) await workouts.bulkWrite(ops, { ordered: false });
