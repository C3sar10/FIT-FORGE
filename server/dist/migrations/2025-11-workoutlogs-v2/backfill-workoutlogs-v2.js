...
function convertLegacyExerciseList(legacyList) {
    ...
    return legacyList.map((item, index) => {
        ...
        if (typeof item === 'string') {
            ...
            return {
                exerciseId: new mongodb_1.ObjectId(item),
        ...
        return {
            exerciseId: item.exerciseId ? new mongodb_1.ObjectId(item.exerciseId) : new mongodb_1.ObjectId(),
...
function v2PatchForWorkoutLog(doc) {
    ...
    if (doc.workoutDetails && doc.workoutDetails.exerciseList) {
        const convertedList = convertLegacyExerciseList(doc.workoutDetails.exerciseList);
...
(async () => {
    ...
    while (true) {
        ...
        if (ops.length)
            await workoutlogs.bulkWrite(ops, { ordered: false });
