...
(async () => {
    ...
    const workoutlogs = mongoose_1.default.connection.db.collection("workoutlogs");
    ...
    const shapeReport = await workoutlogs
        ...
        { $unwind: { path: "$workoutDetails.exerciseList", preserveNullAndEmptyArrays: true } },
        {
            $project: {
                _id: 0,
                exerciseIdType: { $type: "$workoutDetails.exerciseList.exerciseId" },
                nameType: { $type: "$workoutDetails.exerciseList.name" },
                plannedSetsType: { $type: "$workoutDetails.exerciseList.plannedSets" },
                plannedRepsType: { $type: "$workoutDetails.exerciseList.plannedReps" },
