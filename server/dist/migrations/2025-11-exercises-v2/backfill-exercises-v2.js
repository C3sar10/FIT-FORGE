...
function v2PatchFor(doc) {
    ...
    patch.details = { ...d };
    if (isNil(d.repType))
        patch.details.repType = "number";
    if (isNil(d.repNumber))
        patch.details.repNumber = 0;
    if (isNil(d.repRange))
        patch.details.repRange = { min: null, max: null };
    ...
    if (isNil(d.equipment))
        patch.details.equipment = [];
    return patch;
}
(async () => {
